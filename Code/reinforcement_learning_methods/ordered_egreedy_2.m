...
%       * initialEpsilon - initial exploration coefficient
    ...
    while(iteration < totalIterations+1) 
        ...
        %disp(['Iteration ' num2str(iteration) '. WLAN' num2str(wlan_ix) 's turn:'])
        ...
        for wlan_ix_aux = 1 : nWlans
            ...
            if wlan_ix_aux ~= wlan_ix
                %cumulative_reward_per_wlan(wlan_ix_aux) = cumulative_reward_per_wlan(wlan_ix_aux) + rw(wlan_ix_aux);
...
%                 disp(['   - wlan ' num2str(wlan_ix_aux) ' turn'])
%                 disp(['   - cumulative_reward = ' num2str(cumulative_reward_per_wlan(wlan_ix_aux))])
%                 disp(['   - iterations_without_acting = ' num2str(iterations_without_acting(wlan_ix_aux))])
                rewardPerArm(wlan_ix_aux, selectedArm(wlan_ix_aux)) = ...
                    cumulative_reward_per_wlan(wlan_ix_aux) / iterations_without_acting(wlan_ix_aux);
                iterations_without_acting(wlan_ix_aux) = 1;
                cumulative_reward_per_wlan(wlan_ix_aux) = 0;
            end        
        end
        ...
        %disp(['   * Throughput experienced: ' num2str(tptAfterAction)])
        %disp(['   * Reward: ' num2str(rw)])
        
        % Update the exploration coefficient according to the inputted mode
        if updateMode == UPDATE_MODE_FAST
            epsilon(wlan_ix) = initialEpsilon / iterations_per_wlan(wlan_ix);    
        elseif updateMode == UPDATE_MODE_SLOW
            epsilon(wlan_ix) = initialEpsilon / sqrt(iterations_per_wlan(wlan_ix));   
        else
            disp(['updateModeEpsilon = ' num2str(epsilon) ' does not exist!'])
        end
        
        %disp(['   * Updated epsilon: ' num2str(epsilon)])
        %disp(['   * iterations: ' num2str(iterations_per_wlan)])
