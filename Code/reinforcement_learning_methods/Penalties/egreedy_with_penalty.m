...
function [ tptExperiencedPerWlan, timesArmHasBeenPlayed, regretExperiencedPerWlan, meanRewardPerAction ] = ...
    egreedy_with_penalty( wlans, initialEpsilon, upperBoundThroughputPerWlan, alpha_penalty, varargin )
...
%       * initialEpsilon - initial exploration coefficient
    ...
    while(iteration < totalIterations + 1) 
        ...
        for wlan_i = 1 : nWlans      
            ...
            rewardPerArm(wlan_i, selectedArm(wlan_i)) = rw(wlan_i)  - alpha_penalty * (wlansAux(wlan_i).TxPower / max(txPowerActions));
