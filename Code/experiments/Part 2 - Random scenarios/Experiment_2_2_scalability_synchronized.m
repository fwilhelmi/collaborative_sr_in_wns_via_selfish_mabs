...
%%% Thompson sampling in random scenarios. 
...
disp('----------------------------------------------')
disp('SIM_2_2: Random scenarios (sequential approach)')
disp('----------------------------------------------')
...
for s = 1 : size(nWlans, 2)    
    ...
    for scenario = 1 : totalScenarios
        ...
        throughputEvolutionPerWlanOEgreedy{s, scenario}  = ...
            sequential_egreedy(wlans{s,scenario}, initialEpsilon, upperBoundThroughputPerWlan, ...
        ...
        throughputEvolutionPerWlanOExp3{s, scenario}  = ...
            sequential_exp3(wlans{s,scenario}, gamma, initialEta, upperBoundThroughputPerWlan, ...
            	nChannels, ccaActions, txPowerActions);

        % Compute the throughput evolution for the UCB APPROACH
        throughputEvolutionPerWlanOUcb{s, scenario}  = ...
            sequential_ucb(wlans{s,scenario}, upperBoundThroughputPerWlan, ...
            	nChannels, ccaActions, txPowerActions);

        % Compute the throughput evolution for the TS APPROACH
        throughputEvolutionPerWlanOTs{s, scenario}  = ...
            sequential_thompson_sampling(wlans{s,scenario}, upperBoundThroughputPerWlan, ...
            	nChannels, ccaActions, txPowerActions);
