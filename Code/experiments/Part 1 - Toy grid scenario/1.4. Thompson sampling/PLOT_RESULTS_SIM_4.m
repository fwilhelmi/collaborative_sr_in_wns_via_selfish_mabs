...
disp('----------------------------------------------')
disp('PLOT RESULTS OF EXPERIMENT 4')
disp('----------------------------------------------')
...
num_wlans = size(tpt_evolution_per_wlan_ts, 2);
permanent_throughput = tpt_evolution_per_wlan_ts(permanentInterval,:);
...
axis([0 700 0 5000])
% Save Figure
figName = 'experiment_4_hist_throughput';
...
xlabel('TS','fontsize', 24)
set(gca,'FontSize', 22)
% Save Figure
figName = 'experiment_4_boxplot';
savefig(['./Output/' figName '.fig'])
saveas(gcf,['./Output/' figName],'png')
close
...
legend([hBar, h2],{'TS', 'Optimal (PF)'});
% Save Figure
figName = 'experiment_4_mean_tpt';
savefig(['./Output/' figName '.fig'])
saveas(gcf,['./Output/' figName],'png')
close 
...
% Save Figure
figName = 'experiment_4_variability';
savefig(['./Output/' figName '.fig'])
saveas(gcf,['./Output/' figName],'png')