...
%%% performance achieved by applying UCB.
...
disp('************************************************************************')
disp('-----------------------')
disp('SIM_1_2_3: UCB (Toy scenario)')
disp('-----------------------')
...
[tpt_evolution_per_wlan_concurrent_ucb, times_arm_has_been_played_concurrent_ucb, ~] = ...
    concurrent_ucb(wlans, upperBoundThroughputPerWlan);
% Compute the throughput experienced per WLAN at each iteration (sequential)
[tpt_evolution_per_wlan_sequential_ucb, times_arm_has_been_played_sequential_ucb, ~] = ...
    sequential_ucb(wlans, upperBoundThroughputPerWlan);

% Plot the results
if plotResults
    display_results_individual_performance(wlans, tpt_evolution_per_wlan_concurrent_ucb, ...
        times_arm_has_been_played_concurrent_ucb, max_max_min, agg_tpt_max_pf, 'UCB_concurrent');
    display_results_individual_performance(wlans, tpt_evolution_per_wlan_sequential_ucb, ...
        times_arm_has_been_played_sequential_ucb, max_max_min, agg_tpt_max_pf, 'UCB_sequential');
end

% Save the workspace
save('./Output/simulation_1_2_3_workspace.mat')