...
%%% performance achieved by applying UCB in a dynamic scenario.
...
disp('-------------------------------------')
disp('SIM 1_3_2_3: dynamic UCB (sequential)')
disp('-------------------------------------')
...
[tpt_evolution_per_wlan_ucb_stage_1, times_arm_has_been_played_ucb_stage_1, ...
    regret_per_wlan_ucb_stage_1, mean_reward_per_wlan_stage_1] = ...
    sequential_ucb_with_memory(wlans_stage_1, upperBoundThroughputPerWlanStage1, 1, 2500);
tpt_evolution_per_wlan_ucb_stage_1 = [tpt_evolution_per_wlan_ucb_stage_1 zeros(2500,1) zeros(2500,1)];

%% STAGE 2
% Compute the maximum achievable throughput per WLAN
upperBoundThroughputPerWlanStage2 = compute_max_selfish_throughput( wlans_stage_2 );
% Compute the throughput experienced per WLAN at each iteration
[tpt_evolution_per_wlan_ucb_stage_2, times_arm_has_been_played_ucb_stage_2, ...
    regret_per_wlan_ucb_stage_2, mean_reward_per_wlan_stage_2] = ...
    sequential_ucb_with_memory(wlans_stage_2, upperBoundThroughputPerWlanStage2, 2500, 5000);
tpt_evolution_per_wlan_ucb_stage_2 = [tpt_evolution_per_wlan_ucb_stage_2 zeros(2501,1)];

%% STAGE 3
% Compute the maximum achievable throughput per WLAN
upperBoundThroughputPerWlanStage3 = compute_max_selfish_throughput( wlans_stage_3 );
% Compute the throughput experienced per WLAN at each iteration
[tpt_evolution_per_wlan_ucb_stage_3, times_arm_has_been_played_ucb_stage_3, ...
    regret_per_wlan_ucb_stage_3, mean_reward_per_wlan_stage_3] = ...
    sequential_ucb_with_memory(wlans_stage_3, upperBoundThroughputPerWlanStage3, 5000, 10000);

% Adapt results for plotting the results
times_arm_has_been_played_ucb_stage_1 = [times_arm_has_been_played_ucb_stage_1; zeros(1, K); zeros(1,K)];
times_arm_has_been_played_ucb_stage_2 = [times_arm_has_been_played_ucb_stage_2; zeros(1, K)];
tpt_evolution_per_wlan_ucb = [tpt_evolution_per_wlan_ucb_stage_1; ...
    tpt_evolution_per_wlan_ucb_stage_2; tpt_evolution_per_wlan_ucb_stage_3]; 
times_arm_has_been_played_ucb = times_arm_has_been_played_ucb_stage_1 + ...
    times_arm_has_been_played_ucb_stage_2 + times_arm_has_been_played_ucb_stage_3; 
...
axes;
axis([1 20 30 70]);
agg_tpt_per_iteration = sum(tpt_evolution_per_wlan_ucb(1:totalIterations, :), 2);
plot(1 : totalIterations, agg_tpt_per_iteration)
set(gca,'FontSize', 22)
xlabel(['UCB iteration'], 'fontsize', 24)
...
% Save Figure
fig_name = ['temporal_aggregate_tpt_dynamic_scenario_ordered_UCB'];
...
%     times_arm_has_been_played_eg, max_max_min, agg_tpt_max_pf, 'UCB');
   
% Save the workspace
save('./Output/simulation_1_4_2_3_workspace.mat')